...
- `test:electron`: 진단 1경로와 일반 개발·빌드·패키지 3경로를 검사합니다. Unit 3.7에서는 4/4를 통과했고, A형 4지 정답과 B형 5지 오답의 가림·선택·공개·채점까지 자동으로 확인합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·텍스트와 위치 분석 가능·프로파일 미지원·접힌 문서 정보·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. Unit 3.7 재검증은 18/18 통과했고 모든 창 종료·종료 코드 0·포트 해제를 확인했습니다. OPEN-09는 이전에 관찰된 간헐 GPU 종료 진단으로 별도 미해결 상태를 유지합니다.
