...
**Unit 2.3의 제목 키워드 탐색**을 구현했습니다. `text-usable`인 현재 페이지에서 `해설/풀이/정답/답/Answer/Solution/Explanation`의 제목 문맥 후보를 찾고, 일반 화면에는 원문 대신 후보 개수만 안내합니다. 분리된 Text Item과 고정 오탐 사례를 검증했으며 기존 원문 Viewer의 페이지 이동, 50–200% 배율과 높이 맞춤은 그대로 사용할 수 있습니다.

버전 0.2.3의 단위·실제 PDF.js·개발·빌드·패키지·실제 Windows 선택 창 검사는 통과했습니다. 종료 반복은 기존 OPEN-09가 개발 모드 즉시 종료 2회에서 재현되어 16/18이므로 **전체 무오류 완료 판정은 보류**합니다. 18회 모두 종료 코드와 포트 정리는 정상입니다. Unit 1.0과 Unit 2.5도 미착수 상태를 유지합니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
1. 앱을 열고 `연결 완료`·`Unit 2.3 · 제목 키워드 탐색`을 확인합니다.
2. 본문이 충분하고 줄 시작에 `해설:`, `풀이`, `정답: ③`, `Answer: B`, `Solution - ...`, `Explanation ...` 중 하나가 있는 PDF를 엽니다. 앱 상태의 `키워드 후보`가 찾은 개수를 표시해야 합니다.
3. `정답을 고르시오`, 문장 중간의 `Answer`, `Answer choices`만 있는 페이지에서는 후보가 없다고 표시되어야 합니다.
4. 빈 페이지나 글자 없이 그림·도형만 있는 페이지에서는 Viewer를 계속 사용할 수 있고 텍스트와 키워드 분석이 보류되어야 합니다. 페이지 번호 수준의 매우 짧은 텍스트도 같은 방식으로 보류하며 스캔 PDF로 단정하지 않습니다.
5. 후보가 있어도 키워드 원문·전체 문장·좌표 사각형·해설/정답 영역이 별도 화면에 나타나지 않아야 합니다. bbox Debug Overlay는 미완료 Unit 2.5 범위입니다.
6. 여러 페이지를 빠르게 이동하면 마지막으로 표시한 페이지의 후보 상태만 남아야 합니다. 같은 페이지에서 확대·축소·높이 맞춤을 사용해도 원문 Viewer와 상태가 유지되어야 합니다.
7. 다른 PDF로 교체한 뒤 이전 페이지의 후보 수가 나타나지 않는지 확인합니다. footer에는 `PDF를 열었습니다. 원본 파일은 변경하지 않았습니다.`가 유지되어야 합니다.
8. 후보 수가 있어도 가림·문제 선택·채점이 활성화되지 않는지 확인합니다. 기존 처음·이전·번호·다음·마지막·좌우 버튼, 50–200% 배율, 높이 맞춤과 잘못된 페이지 안내는 이전처럼 동작해야 합니다.
...
- `npm test`: 81개. 기존 경계·텍스트 품질·좌표 검증에 더해 일곱 키워드, 분리된 항목, 문맥·오탐, 보류·실패 계약과 실제 PDF.js 후보 1개·원본 해시를 확인합니다.
- `test:electron`: 개발·빌드·패키지 3경로에서 후보 1개/없음/보류 상태와 원문 DOM 비노출, 기존 Viewer·오프라인 자산·입력·보안 회귀를 검사합니다. 실제 읽기 권한 거부는 **생성한 시험 파일 하나의 권한만 임시 변경하고 복원**합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·텍스트와 위치 분석 가능·키워드 후보 없음·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. 최종 Unit 2.3 실행은 개발 즉시 종료 2회에서 기존 GPU 오류가 재현되어 개발 7/9·패키지 9/9입니다. 모든 창 종료·종료 코드·포트 정리는 정상입니다.
...
src/analysis/   페이지 Text Content 품질·PDF user space bbox·제목 키워드 후보
...
Unit 1.0의 전체 샘플 행렬과 Unit 2.5 Debug Overlay, 영역·지원 판정·CBT는 미구현입니다. 현재 bbox는 글리프 윤곽·잉크·클리핑을 보장하지 않는 근사 축 정렬 사각형이며 실제 화면과 sourceIndex별 시각 대조는 아직 없습니다. 키워드는 원래 item 순서와 `hasEOL`만 사용한 초기 제목 문맥 규칙이며 시각적 줄/블록·영역·정답이나 CBT 지원을 뜻하지 않습니다. 최소 비공백 12자·판독 가능 비율 0.8도 초기 보류선입니다. 사용자 회전·Text Layer는 없습니다. Canvas backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한합니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
