...
**Unit 1.1의 PDF 파일 선택·기본 검사 기능**을 구현했습니다. 이름과 크기만 표시하며 PDF 페이지 표시·드롭·문제풀이는 아직 없습니다.

파일 선택 검사는 통과했지만 기존 Unit 0.4의 빠른 종료 GPU 오류가 재현되어 **전체 무오류 완료 판정은 보류**합니다. Unit 1.0도 미착수 상태를 유지합니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
1. 앱을 열고 `연결 완료`·`Unit 1.1 · PDF 파일 선택`을 확인합니다.
2. `PDF 선택`으로 로컬 PDF 한 개를 고르면 이름과 크기가 표시됩니다. **PDF 내용이 보이지 않는 것은 현재 범위에 맞는 동작**입니다.
3. `다른 PDF 선택`을 누른 뒤 취소합니다. 이전 이름·크기를 유지하고 취소 안내가 나와야 합니다.
4. 다른 PDF를 선택하면 이름·크기가 바뀝니다. 선택 창이 열려 있거나 검사 중이면 버튼은 잠시 비활성화됩니다.
5. 별도 시험 파일로 빈 파일, 텍스트를 PDF로 이름만 바꾼 파일, 50 MiB 초과 파일을 검사합니다. 실패 안내와 이전 선택 유지가 정상입니다. 개인 원본을 바꿀 필요는 없습니다.
6. 한글·공백·대문자 `.PDF` 이름과 작은 창을 확인합니다. 파일명·버튼·상태가 겹치지 않고 아래 안내까지 스크롤되어야 합니다.
7. 창을 닫고 다시 열면 선택은 초기화됩니다. 앱은 파일 이름·경로를 저장하거나 최근 문서 목록에 추가하지 않습니다. Windows 선택 창 자체의 기록까지 통제한다는 뜻은 아닙니다.
...
- `npm test`: 36개. 자산·CSP 경계, PDF 기본 검사, 원본 해시, 크기 경계, IPC 발신자·취소·중복·오류·핸들 정리를 확인합니다.
- `test:electron`: 개발·빌드·패키지 3경로에서 실제 IPC·파일 읽기·화면·보안·앱 전용 오프라인 조건을 검사합니다. 선택 결과는 도구가 제공합니다. 패키지의 실제 읽기 권한 거부는 **생성한 시험 파일 하나의 권한만 임시 변경하고 복원**해 확인합니다. 개인 파일 권한은 바꾸지 않습니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 합성 PDF 선택·취소와 원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. 이번 18회 중 1회에서 기존 GPU 오류가 재현돼 실패했습니다. 종료 코드와 개발 포트 정리는 정상입니다.
...
```text
electron/       창·preload·자산 프로토콜·요청 제한·PDF 선택/기본 검사
src/ui/         실행 상태·파일 선택 상태 표시
...
renderer에는 `runtimeInfo`와 인자 없는 `selectPdfFile()`만 노출합니다. 범용 파일 읽기·쓰기나 원시 IPC, 원본 경로·내용은 제공하지 않습니다. 선택 정보는 화면 메모리에만 있으며 원본 PDF는 수정하지 않습니다.

Unit 1.0의 전체 샘플 행렬, 1.2 드롭, 1.3 PDF.js 렌더링과 이후 기능은 미구현입니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 실제 Git 초기화·커밋은 하지 않았습니다.
