...
**Unit 1.3의 로컬 PDF.js 첫 페이지 렌더링**을 구현했습니다. 선택하거나 드롭한 PDF의 첫 페이지와 전체 페이지 수를 표시하며 페이지 이동·배율·문제풀이는 아직 없습니다.
...
1. 앱을 열고 `연결 완료`·`Unit 1.3 · PDF.js 첫 페이지 렌더링`을 확인합니다.
2. `PDF 선택`으로 한글과 이미지가 있는 작은 로컬 PDF 한 개를 고릅니다. 첫 페이지가 Canvas에 보이고 상태 패널에 `1 / 전체 페이지 수`가 표시되어야 합니다.
3. Windows 탐색기에서 다른 정상 PDF 한 개를 문서 작업 공간에 드롭합니다. 첫 페이지·이름·크기·페이지 수가 새 문서로 바뀌어야 합니다.
4. 암호가 필요한 PDF 복사본을 선택합니다. 암호 입력이나 원문 대신 현재 미지원 안내가 나오고 이전 Canvas가 보이지 않아야 합니다.
5. 구조가 손상된 별도 합성 PDF를 선택합니다. 손상/지원 불가 안내가 나오고 이전 Canvas가 보이지 않아야 합니다.
6. PDF 두 개를 함께 드롭하거나 폴더·웹 주소·비PDF를 입력합니다. 거절 안내가 나오고 직전의 정상 문서를 유지해야 합니다.
7. `다른 PDF 선택`을 누른 뒤 취소합니다. 이전 문서와 Canvas를 유지하고 취소 안내가 나와야 합니다.
8. 필요하면 원본 PDF의 수정 시각·크기·해시를 전후 비교합니다. 앱은 원본을 바꾸지 않으며 창을 닫고 다시 열면 선택이 초기화됩니다.
...
- `npm test`: 47개. 자산·CSP 경계, 읽기 전용 전체 바이트·원본 해시·크기 경계, 선택/드롭 IPC, PDF 어댑터의 첫 페이지·오류 분류·취소/정리를 확인합니다.
- `test:electron`: 개발·빌드·패키지 3경로에서 실제 PDF.js worker, 한글·포함 이미지 Canvas 픽셀, 암호·손상 상태, 로컬 CMap·ICC·표준 글꼴·WASM, 파일 입력·보안·앱 전용 오프라인 조건을 검사합니다. 실제 읽기 권한 거부는 **생성한 시험 파일 하나의 권한만 임시 변경하고 복원**합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·첫 페이지 렌더·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. Unit 1.3의 18회 중 개발 모드 즉시 종료 2회에서 기존 GPU 오류가 재현돼 실패했습니다. 모든 종료 코드와 개발 포트 정리는 정상입니다.
...
src/pdf/        PDF.js 첫 페이지 렌더·요청 취소·자원 정리
src/ui/         실행·파일 입력·Canvas 상태 표시
...
Unit 1.0의 전체 샘플 행렬과 Unit 1.4 페이지 이동 이후 기능은 미구현입니다. 현재는 첫 페이지만 원문으로 표시하며 확대·너비 맞춤·Text Layer·분석·CBT는 없습니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
