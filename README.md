...
1. 두 페이지 이상인 PDF를 열고 문제 페이지에서 `해설·정답 영역 설정`으로 두 영역을 확정합니다.
2. 다른 페이지로 이동해 그 페이지의 해설·정답 영역도 확정합니다. 오른쪽 `문제·해설 페이지 연결`에 문제 페이지 번호를 입력하고 `현재 페이지로 연결`을 누릅니다.
3. `페이지 간 연결을 확정했습니다. 기존 CBT 상태는 바꾸지 않았습니다.`가 표시되는지 확인합니다. 다시 다른 확정 페이지에서 연결하면 `수정했습니다`가 표시되어야 합니다.
4. `연결 해제`를 누릅니다. 연결 해제 안내가 표시되고, 두 페이지의 기존 수동 영역·가림·선택·채점 상태가 바뀌지 않아야 합니다.
5. 문제와 같은 페이지를 연결하려 하거나, 어느 한쪽에 확정 영역이 없으면 오류 안내가 표시되고 기존 연결이 유지되어야 합니다.
