...
**Unit 2.2의 Text Item 좌표 분석**을 구현했습니다. 현재 페이지에서 추출한 텍스트 항목을 회전 전 PDF user space의 근사 bbox로 바꾸고 viewBox·`userUnit`·고유 회전을 반영해 viewport CSS 좌표로 투영할 수 있습니다. 일반 화면에는 원문이나 bbox를 표시하지 않고 텍스트와 위치의 분석 가능 여부만 안내합니다. 기존 원문 Viewer의 페이지 이동, 50–200% 배율과 높이 맞춤은 그대로 사용할 수 있습니다.

버전 0.2.2의 단위·실제 PDF.js·개발·빌드·패키지·실제 Windows 선택 창 검사는 통과했습니다. 종료 반복은 기존 OPEN-09가 개발 모드 즉시 종료 1회에서 재현되어 17/18이므로 **전체 무오류 완료 판정은 보류**합니다. 18회 모두 종료 코드와 포트 정리는 정상입니다. Unit 1.0도 미착수 상태를 유지합니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
1. 앱을 열고 `연결 완료`·`Unit 2.2 · Text Item 좌표 분석`을 확인합니다.
2. 한글 본문이 충분한 PDF를 열고 앱 상태의 `텍스트 분석`이 `현재 페이지의 텍스트와 위치를 분석할 수 있습니다.`로 바뀌는지 확인합니다.
...
5. 고유 회전이 있는 PDF와 viewBox가 일반적이지 않은 PDF도 원문이 정상 표시되고 텍스트 위치 상태가 확인 가능한지 살펴봅니다. bbox 자체의 화면 표시는 Unit 2.5 범위이므로 현재 일반 화면에 좌표 사각형이 없어야 합니다.
6. 여러 페이지를 빠르게 이동하면 마지막으로 표시한 페이지의 상태만 남아야 합니다. 같은 페이지에서 확대·축소·높이 맞춤을 사용해도 품질·좌표 상태는 유지되어야 합니다.
7. 다른 PDF로 교체한 뒤 이전 페이지의 결과가 나타나지 않는지 확인합니다. footer에는 `PDF를 열었습니다. 원본 파일은 변경하지 않았습니다.`가 유지되어야 합니다.
8. 기존 처음·이전·번호·다음·마지막·좌우 버튼, 50–200% 배율, 높이 맞춤, 고유 회전과 잘못된 페이지 안내가 이전처럼 동작하는지 확인합니다.
...
- `npm test`: 70개. 기존 경계와 PageTextSource/Assessment에 더해 가로·세로·회전·반사 bbox, 실패 코드, PDF ↔ viewport 왕복, offset viewBox·`UserUnit 2`·0/90/180/270도·50/100/200%의 실제 PDF.js 대조와 원본 해시를 확인합니다.
- `test:electron`: 개발·빌드·패키지 3경로에서 텍스트·위치 분석 가능/보류 상태와 원문 DOM 비노출, 기존 Viewer·오프라인 자산·입력·보안 회귀를 검사합니다. 실제 읽기 권한 거부는 **생성한 시험 파일 하나의 권한만 임시 변경하고 복원**합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·텍스트와 위치 분석 가능·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. 최종 Unit 2.2 실행은 개발 즉시 종료 1회에서 기존 GPU 오류가 재현되어 개발 8/9·패키지 9/9입니다. 모든 창 종료·종료 코드·포트 정리는 정상입니다.
...
src/analysis/   페이지 Text Content 품질 분류·PDF user space TextItemRecord bbox
src/shared/     PageTextSource v1 검증과 계약 버전
src/ui/         실행·파일 입력·페이지 이동·배율·Canvas·텍스트/위치 상태 표시
...
Unit 1.0의 전체 샘플 행렬과 Unit 2.5 이후 기능은 미구현입니다. 현재 bbox는 글리프 윤곽·잉크·클리핑을 보장하지 않는 근사 축 정렬 사각형이며 실제 화면과 sourceIndex별 시각 대조는 아직 없습니다. 최소 비공백 12자·판독 가능 비율 0.8도 초기 보류선입니다. 사용자 회전·Text Layer·Debug Overlay·키워드·영역·CBT는 없습니다. Canvas backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한합니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
