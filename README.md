...
**Unit 2.7.2 Viewer 전환·높이 맞춤 안정화**를 구현했습니다. 기본 창 Shell과 접을 수 있는 문서 정보, 상시 분석 요약을 유지하면서 페이지 이동 알림이 PDF 영역을 밀지 않게 했습니다. 높이 맞춤 상태의 창 크기 변경은 실제 목표 배율이 달라질 때만 다시 렌더하고, 새 결과가 준비될 때까지 기존 Canvas를 유지합니다. 기존 원문 Viewer와 명시적 개발 진단 Overlay, Unit 2.6 분석 판정은 그대로 유지됩니다.

버전 0.2.7의 단위·실제 PDF.js·개발·빌드·패키지·실제 Windows 선택 창 검사는 통과했습니다. 페이지 이동 중 page stage 위치·높이 변화 1 CSS px 이하와 높이 맞춤 완료 뒤 450ms 동안 추가 Canvas 교체가 없음을 개발·빌드·패키지에서 확인했습니다. 종료 반복은 개발 7/9·패키지 9/9이며 개발 즉시 종료 두 번에서 기존 OPEN-09가 재현되어 **전체 무오류 완료 판정은 보류**합니다. 모든 18회는 창 종료·종료 코드·포트 정리가 정상입니다. Unit 1.0은 미착수이며 Phase 3 기능은 시작하지 않았습니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
1. `npm run dev`로 일반 앱을 열어 footer의 `Unit 2.7.2 · Viewer 안정화`를 확인합니다. 기본 창에서 앱 바깥쪽 세로 스크롤 없이 header와 footer가 동시에 보여야 합니다.
...
4. PDF를 열고 `높이 맞춤`을 누른 뒤 2초 이상 기다립니다. PDF 크기나 위치가 스스로 반복해 바뀌거나 로딩 문구가 반복되면 안 됩니다. 이전/다음을 빠르게 눌러도 로딩 안내가 PDF 작업 영역을 아래로 밀지 않고, 기존 화면에서 새 페이지로 한 번에 바뀌어야 합니다.
5. `높이 맞춤` 상태에서 창을 늘이거나 줄인 뒤 놓습니다. 조절 중 검은색·빈 화면이 보이지 않고, 새 높이에 맞춰진 뒤 반복 렌더 없이 정지해야 합니다. PDF가 길면 Viewer 안에서, 상태가 길면 오른쪽 사이드 안에서 스크롤되어야 합니다.
6. 창을 640×480까지 줄여 한 열 배치와 main 내부 스크롤로 모든 영역에 접근할 수 있고 가로 넘침이 없는지 확인합니다.
7. PDF 교체 뒤 이전 판정이 남지 않는지 확인합니다. 전체 문제·해설·정답 문장, 파일 경로와 좌표가 상태 영역에 추가로 노출되지 않아야 합니다.
8. 좌표 진단이 필요할 때만 `npm run dev:debug`를 사용합니다. 패키지는 PowerShell에서 `& '.\release\local-pdf-cbt-win32-x64\local-pdf-cbt.exe' --debug-overlay`로 진단 모드를 열 수 있습니다. 일반/진단 모두 Mask·답 확인·채점은 나타나지 않아야 합니다.
...
- `test:electron`: 진단 1경로와 일반 개발·빌드·패키지 3경로, 총 4개를 검사합니다. 일반 경로는 기본/최소 창 스크롤 소유권, compact header, 문서 정보 키보드 토글, 분석 상시 노출, 페이지 이동·높이 맞춤 resize 중 화면 Canvas 비초기화, page stage 좌표 안정성과 렌더 완료 뒤 450ms 정지 상태를 확인합니다. 기존 프로파일·Viewer·오프라인·입력·보안 회귀와 진단 좌표 대조도 유지합니다. 실제 읽기 권한 거부는 **생성한 시험 파일 하나의 권한만 임시 변경하고 복원**합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·텍스트와 위치 분석 가능·프로파일 미지원·접힌 문서 정보·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. Unit 2.7.2 결과는 개발 7/9·패키지 9/9이며 개발 즉시 종료 두 번에서 기존 OPEN-09 GPU 진단이 재현됐습니다. 모든 창 종료·종료 코드·포트 정리는 정상입니다.
