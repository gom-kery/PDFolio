...
**Unit 1.5의 PDF 확대·축소·너비 맞춤**을 구현했습니다. 선택하거나 드롭한 PDF를 한 페이지씩 표시하며 처음·이전·번호 입력·다음·마지막 이동, 50–200% 배율과 너비 맞춤을 사용할 수 있습니다. 문제풀이는 아직 없습니다.

Unit 1.5의 단위·개발·빌드·패키지·실제 Windows 선택 창 검사는 통과했습니다. 최종 종료 반복은 기존 OPEN-09가 개발 모드 즉시 종료 3회에서 재현되어 15/18이므로 **전체 무오류 완료 판정은 보류**합니다. 패키지 종료 9/9, 모든 종료 코드와 포트 정리는 정상입니다. Unit 1.0도 미착수 상태를 유지합니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
1. 앱을 열고 `연결 완료`·`Unit 1.5 · PDF 확대·축소·너비 맞춤`을 확인합니다.
2. `PDF 선택`으로 2페이지 이상인 작은 로컬 PDF를 엽니다. 완료 문구가 PDF 위에 반복되지 않고 footer에 `PDF를 열었습니다. 원본 파일은 변경하지 않았습니다.`가 표시되어야 합니다.
3. PDF 좌우 버튼과 PDF 아래의 처음·이전·번호·다음·마지막 버튼을 사용합니다. 첫/마지막 페이지에서 같은 방향의 두 버튼이 함께 비활성화되어야 합니다.
4. `−`·`+`로 50%와 200%까지 이동합니다. 배율과 PDF 크기가 함께 바뀌고 경계 버튼이 비활성화되어야 합니다.
5. `너비 맞춤`을 누르고 창 폭을 바꿉니다. PDF가 사용 가능한 폭에 다시 맞고 최소 창에서는 좌우 버튼이 사라지되 하단 이동 도구가 남아야 합니다.
...
- `npm test`: 49개. 자산·CSP 경계, 읽기 전용 전체 바이트·원본 해시·크기 경계, 선택/드롭 IPC, PDF 어댑터의 페이지·배율 경계·fit width·고유 회전·Canvas 상한·최신 렌더·오류 분류·취소/정리를 확인합니다.
- `test:electron`: 개발·빌드·패키지 3경로에서 페이지 이동·좌우/하단 버튼 상태·50/200% 배율·너비 맞춤·창 크기 재계산·고유 회전·footer 안내와 실제 PDF.js worker, 한글·포함 이미지 Canvas 픽셀, 암호·손상 상태, 로컬 자산, 파일 입력·보안·앱 전용 오프라인 조건을 검사합니다. 실제 읽기 권한 거부는 **생성한 시험 파일 하나의 권한만 임시 변경하고 복원**합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·첫 페이지 렌더·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. 최종 Unit 1.5 실행은 개발 모드 즉시 종료 3회에서 기존 GPU 오류가 재현되어 15/18입니다. 패키지 9/9와 개발 모드 250ms·1500ms 6/6, 모든 종료 코드·포트 정리는 정상입니다.
...
src/pdf/        PDF.js 현재 페이지·배율·고유 회전·Canvas 상한·최신 요청·자원 정리
...
Unit 1.0의 전체 샘플 행렬과 Unit 1.6 이후 기능은 미구현입니다. 현재는 선택한 PDF를 한 페이지씩 원문으로 표시하고 50–200% 확대·축소와 너비 맞춤을 제공합니다. 사용자 회전·Text Layer·분석·CBT는 없습니다. Canvas backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한합니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
