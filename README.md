...
**Unit 3.4 해설·정답 공개**를 구현했습니다. PDF를 연 직후에는 전체 가림으로 시작하며, 오른쪽에서 해설과 정답 사각형을 각각 수동 확정한 한 페이지·한 문제만 실제로 가립니다. 답 하나를 선택·확정하면 해당 문제의 해설·정답 가림만 공개되며, 페이지를 떠났다가 돌아와도 세션 동안 유지됩니다. 정답 추출·정답 비교·채점은 아직 없습니다.

버전 0.3.4의 Node 116개, 실제 PDF.js, 진단·개발·빌드 Electron 검사는 통과했습니다. 이전 패키지 앱이 실행 중이라 새 Windows 패키지 생성과 패키지 경로 재검증은 대기 중입니다. Unit 1.0은 미착수이며 OPEN-09 GPU 종료 진단은 별도 미해결입니다. 다음 계획 Unit은 3.5 정답 추출입니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
9. 정답 값, 맞음/틀림, 채점 결과는 아직 나타나지 않아야 합니다. `npm run dev:debug`에서는 CBT 가림·수동 설정·답 선택 카드가 모두 숨겨져야 합니다.
...
- `npm test`: 116개. 기존 검사에 잠긴 선택과 같은 Question/revision만 공개하고, 재방문 상태·context 불일치·세션 폐기를 더했습니다.
...
Unit 1.0의 전체 샘플 행렬은 미구현입니다. Unit 3.4는 한 페이지·한 문제, 해설·정답 사각형 각 하나와 4/5지 단일 선택·확정·공개만 지원하며 여러 문제·여러 열·여러 영역·여러 페이지 연결과 영구 저장은 지원하지 않습니다. 사용자가 직접 범위를 확인해야 하고 자동 후보는 확정을 대신하지 않습니다. 공개는 화면 메모리에서 같은 Question의 가림만 숨기며 정답 값을 추출하거나 답을 비교·채점하지 않습니다. 수동 영역 재편집·파일 교체·새로고침·종료 때 상태는 사라집니다. 사각형은 포인터로만 만들 수 있어 키보드 좌표 입력·손잡이 크기 조절·터치 실기기 검증이 남았습니다. 640×480에서는 앱 main 내부 스크롤이 필요하며 실제 Windows 200% 디스플레이 배율·스크린 리더는 검증하지 않았습니다. 사용자 회전·Text Layer는 없습니다. Canvas backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한합니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
