...
**Unit 3.1 해설·정답 Mask Layer**를 구현했습니다. PDF를 연 직후에는 전체 가림으로 시작하며, 오른쪽에서 해설과 정답 사각형을 각각 수동 확정한 한 페이지·한 문제만 실제로 가립니다. 영역을 설정하는 동안에만 원문과 정답이 보일 수 있습니다. 답 선택·공개·채점은 아직 없습니다.

버전 0.3.1의 Node 111개, 실제 PDF.js, 개발·빌드·패키지 Electron 4경로와 실제 Windows 선택 창 검사는 통과했습니다. 전체 가림, 설정 화면, 확정 뒤 두 영역 가림, 확대·높이 맞춤·파일 교체를 확인했습니다. Unit 1.0은 미착수이며 OPEN-09 GPU 종료 진단은 별도 미해결입니다. 다음 계획 Unit은 3.2 객관식 답 선택입니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
5. `확정 영역 수정`을 눌러 설정 화면으로 돌아갔다가 `취소`하면 이전 두 영역 가림이 복원되는지 확인합니다.
6. 답 선택, 답 확인, 해설 공개, 정답 공개, 채점 버튼은 아직 나타나지 않아야 합니다. `npm run dev:debug`에서는 CBT 가림과 수동 설정 카드가 숨겨져야 합니다.
...
- `npm test`: 111개. 기존 검사에 확정 Question/Region의 revision·페이지·영역·겹침 검증과 준비 전 차단을 더했습니다.
- `test:electron`: 진단 1경로와 일반 개발·빌드·패키지 3경로, 총 4개를 검사합니다. 일반 경로는 전체 가림, 설정 중 원문 표시, 확정 뒤 두 영역 가림, Canvas 접근성 차단·Text Layer 부재, 확대·높이 맞춤·파일 교체와 원본 불변을 확인합니다.
...
Unit 1.0의 전체 샘플 행렬은 미구현입니다. Unit 3.1은 한 페이지·한 문제, 해설·정답 사각형 각 하나만 지원하며 여러 문제·여러 열·여러 영역·여러 페이지 연결과 영구 저장은 지원하지 않습니다. 사용자가 직접 범위를 확인해야 하고 자동 후보는 확정을 대신하지 않습니다. 답 선택·확인·공개·정답 추출·채점은 이후 Unit 범위입니다. 사각형은 포인터로만 만들 수 있어 키보드 좌표 입력·손잡이 크기 조절·터치 실기기 검증이 남았습니다. 현재 확정은 메모리 전용으로 파일 교체·새로고침·종료 때 사라집니다. 640×480에서는 앱 main 내부 스크롤이 필요하며 실제 Windows 200% 디스플레이 배율·스크린 리더는 검증하지 않았습니다. 사용자 회전·Text Layer는 없습니다. Canvas backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한합니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
