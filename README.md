...
**Unit 3.2 객관식 답 선택**을 구현했습니다. PDF를 연 직후에는 전체 가림으로 시작하며, 오른쪽에서 해설과 정답 사각형을 각각 수동 확정한 한 페이지·한 문제만 실제로 가립니다. 이 준비가 끝난 페이지에서만 4지 또는 5지를 고르고 답 하나를 선택할 수 있습니다. 답 확인·공개·정답 추출·채점은 아직 없습니다.

버전 0.3.2의 Node 114개, 실제 PDF.js, 진단·개발·빌드 Electron 검사는 통과했습니다. 이전 패키지 앱이 실행 중이라 새 Windows 패키지 생성과 패키지 경로 재검증은 대기 중입니다. Unit 1.0은 미착수이며 OPEN-09 GPU 종료 진단은 별도 미해결입니다. 다음 계획 Unit은 3.3 답 확인 상태 전이입니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
1. `npm run dev`로 일반 앱을 열고 한 페이지에 한 문제와 해설·정답이 함께 있는 PDF를 엽니다. 처음에는 PDF 전체가 가려지고, `CBT 가림`에 준비되지 않았다는 안내가 보여야 합니다.
2. 오른쪽 `해설·정답 영역 설정`에서 `영역 설정 시작`을 누릅니다. 이 설정 화면에서만 원문과 정답이 보이고, CBT 가림은 잠시 숨겨져야 합니다.
3. `해설`과 `정답`을 각각 지정한 뒤 `가림 미리보기`와 `영역 확정`을 누릅니다. 확정 뒤에는 설정 화면이 닫히고, 두 사각형만 완전히 불투명하게 가려져야 합니다.
4. 확대·축소·높이 맞춤을 실행해도 가림이 같은 PDF 내용 위에 남는지 확인합니다. 다른 페이지로 가거나 다른 PDF를 열면 확정되지 않은 페이지는 다시 전체 가림이어야 합니다.
5. 오른쪽 `답 선택`에서 기본 `4지`와 1\~4번 radio를 확인하고 하나를 선택합니다. `5지`로 바꾸면 이전 선택이 지워지고 1\~5번만 보여야 합니다. 답 하나를 다시 선택한 뒤 확대·축소해도 선택이 유지되어야 합니다.
6. `확정 영역 수정`을 시작하면 `답 선택` 카드는 숨겨져야 합니다. 취소하면 이전 두 영역 가림은 복원되지만 답 선택은 초기 상태로 돌아와야 합니다. 다른 PDF를 열면 카드가 숨겨져야 합니다.
7. 답 확인, 해설·정답 공개, 정답 추출, 채점 버튼은 아직 나타나지 않아야 합니다. `npm run dev:debug`에서는 CBT 가림·수동 설정·답 선택 카드가 모두 숨겨져야 합니다.
...
- `npm test`: 114개. 기존 검사에 4/5지 설정, 단일 선택, 보기 수 변경 초기화, revision별 메모리 격리·무효화를 더했습니다.
- `test:electron`: 진단 1경로와 일반 개발·빌드·패키지 3경로를 검사합니다. 현재 진단·개발·빌드 3경로는 통과했고, 새 패키지 경로 검사는 실행 중인 이전 패키지를 닫은 뒤 다시 실행해야 합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·텍스트와 위치 분석 가능·프로파일 미지원·접힌 문서 정보·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. Unit 3.1 최종 결과는 개발 8/9·패키지 9/9이며 개발 즉시 종료 한 번에서 OPEN-09 GPU 진단이 재현됐습니다. 모든 창 종료·종료 코드 0·포트 해제는 정상이었습니다.
...
src/cbt/        수동 page-single Question/Region 설정·검증·세션 확정·CBT Mask 준비 판정
src/shared/     PageTextSource v1 검증과 계약 버전
src/ui/         실행·Viewer·분석 상태·수동 영역 설정·CBT Mask·명시적 진단 overlay
...
Unit 1.0의 전체 샘플 행렬은 미구현입니다. Unit 3.2는 한 페이지·한 문제, 해설·정답 사각형 각 하나와 4/5지 단일 선택만 지원하며 여러 문제·여러 열·여러 영역·여러 페이지 연결과 영구 저장은 지원하지 않습니다. 사용자가 직접 범위를 확인해야 하고 자동 후보는 확정을 대신하지 않습니다. 답 확인·잠금·공개·정답 추출·채점은 이후 Unit 범위입니다. 선택은 세션 메모리에만 있고 수동 영역 재편집·파일 교체·새로고침·종료 때 사라집니다. 사각형은 포인터로만 만들 수 있어 키보드 좌표 입력·손잡이 크기 조절·터치 실기기 검증이 남았습니다. 640×480에서는 앱 main 내부 스크롤이 필요하며 실제 Windows 200% 디스플레이 배율·스크린 리더는 검증하지 않았습니다. 사용자 회전·Text Layer는 없습니다. Canvas backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한합니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
