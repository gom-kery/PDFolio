...
**Unit 3.6 자동 채점**을 구현했습니다. 사용자가 확정한 한 페이지·한 문제에서 잠긴 선택과 단일 `known` 정답만 비교해 맞음·오답을 표시합니다. 정답이 복수·없음·범위 초과·분석 대기이면 틀림으로 바꾸지 않고 `채점 불가`로 표시합니다.

버전 0.3.6의 Node 122개, 실제 PDF.js, 개발·빌드 Electron 검사는 통과했습니다. 패키지 재검증은 실행 중인 이전 앱을 종료한 뒤 진행합니다. Unit 1.0은 미착수이며 OPEN-09 GPU 종료 진단은 별도 미해결입니다. 다음 계획 Unit은 3.6.5 Viewer 작업 공간·화면 맞춤입니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
5. 오른쪽 `답 선택`에서 기본 `4지`와 1\~4번 radio를 확인합니다. 미선택 상태에서는 `답 확인`이 비활성화되어야 합니다. 하나를 선택하면 활성화되며, `5지`로 바꾸면 이전 선택이 지워지고 1\~5번만 보여야 합니다.
6. 답 하나를 다시 선택한 뒤 `답 확인`을 누릅니다. `n번 선택을 확정했고 해설과 정답을 공개했습니다.`가 보이고, 해당 두 가림이 사라져 해설·정답이 보여야 합니다. 4/5지·답 선택·답 확인은 모두 비활성화되어야 합니다.
7. 다른 페이지로 이동한 뒤 다시 돌아옵니다. 같은 문제의 해설·정답은 계속 공개되고 `해설·정답 공개됨` 상태가 보여야 합니다. 다른 페이지·다른 문제에는 공개 상태가 전파되지 않아야 합니다.
8. `확정 영역 수정`을 시작하면 `답 선택` 카드는 숨겨져야 합니다. 취소하면 이전 두 영역 가림은 복원되고 답 선택은 초기 상태로 돌아와야 합니다. 다른 PDF를 열어도 카드가 숨겨져야 합니다.
9. 정답 영역의 텍스트가 `정답: ④`처럼 단일 값이면 사이드의 `정답 추출`에 값 확인 안내만 보이고, 답 확인 전에 `4번`과 채점 결과가 노출되지 않아야 합니다. 4번을 선택해 답 확인하면 `정답입니다.`가, 다른 번호를 선택한 새 시도에서는 `오답입니다.`와 정답 4번이 보여야 합니다.
10. 단일 값이 없거나 여러 개, 6번 이상, 현재 보기 수보다 큰 값이면 해설·정답은 공개하되 `채점할 수 없습니다.`가 보여야 합니다. 이 결과는 나중 텍스트 분석으로 맞음/오답으로 바뀌면 안 됩니다. `npm run dev:debug`에서는 CBT 가림·수동 설정·답 선택·정답 추출·채점 결과 카드가 모두 숨겨져야 합니다.
