...
**Unit 2.5의 분석 결과 Debug Overlay**를 구현했습니다. 명시적 개발 진단 모드에서 현재 페이지의 Text Item/sourceIndex, 기존 제목 키워드 근거와 해설·정답 영역 후보를 PDF.js Canvas 위에 겹쳐 좌표를 대조할 수 있습니다. 일반 실행에는 진단 panel과 overlay DOM이 없으며 기존 원문 Viewer의 페이지 이동, 50–200% 배율과 높이 맞춤은 그대로 사용할 수 있습니다.

버전 0.2.5의 단위·실제 PDF.js·개발·빌드·패키지·실제 Windows 선택 창 검사는 통과했습니다. 종료 반복은 기존 OPEN-09가 개발 모드 즉시 종료 1회에서 재현되어 17/18이므로 **전체 무오류 완료 판정은 보류**합니다. 18회 모두 종료 코드와 포트 정리는 정상입니다. Unit 1.0은 미착수이며 다음 Unit 2.6도 시작하지 않았습니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
1. 먼저 `npm run dev`로 일반 앱을 열어 footer의 `Unit 2.5 · 분석 결과 Debug Overlay`를 확인합니다. PDF를 열어도 `PDF 좌표 Debug Overlay` panel과 좌표 사각형은 나타나지 않아야 합니다.
2. 일반 앱을 닫고 `npm run dev:debug`로 진단 모드를 엽니다. 패키지는 PowerShell에서 `& '.\release\local-pdf-cbt-win32-x64\local-pdf-cbt.exe' --debug-overlay`로 같은 모드를 열 수 있습니다.
3. 충분한 문제·보기 뒤에 줄 시작 `해설:`과 `정답:`이 한 번씩 있는 단일 열 PDF를 엽니다. Canvas 위 파란 `T{sourceIndex}`, 주황 `K{번호}`, 초록 영역 줄과 보라 점선 `R{번호}`가 나타나고 panel의 Text Item·키워드·영역 개수와 일치하는지 확인합니다.
4. `+`, `-`, `높이 맞춤`을 사용하고 창 크기를 바꿔도 사각형이 PDF 글자와 함께 움직이며 Canvas 밖으로 따로 밀리지 않아야 합니다. Overlay와 Canvas의 CSS 크기가 같아야 합니다.
5. 고유 회전 90°·180°·270° 페이지가 있는 PDF에서는 `T` 사각형도 같은 방향과 위치로 다시 배치되어야 합니다. 앱에는 사용자가 임의 회전하는 기능이 아직 없습니다.
6. `오버레이 숨기기`와 `오버레이 표시`가 원문 Canvas나 분석 상태를 바꾸지 않는지 확인합니다. 페이지 이동·PDF 교체 뒤에는 이전 페이지의 사각형이 남지 않아야 합니다.
7. panel과 좌표 라벨에는 전체 문제·해설·정답 문장이나 파일 경로가 복사되지 않아야 합니다. 진단 모드는 위치를 드러낼 수 있으므로 실제 학습용 일반 실행으로 사용하지 않습니다.
8. 후보가 있어도 Mask·문제 선택·답 확인·채점이 활성화되지 않아야 합니다. 영역이 보이지 않거나 보류되어도 새로운 영역을 추정하거나 지원한다고 판정하지 않습니다.
...
- `npm test`: 97개. 기존 경계·텍스트 품질·좌표·키워드·영역 검증에 더해 원문 없는 Debug Overlay 모델, 100→200% 좌표 재투영, 회전, 보류·계약 불일치를 확인합니다.
- `test:electron`: 진단 1경로와 일반 개발·빌드·패키지 3경로, 총 4개를 검사합니다. 진단 경로는 Text Item·키워드·영역 레이어, 100→125%·높이 맞춤·창 크기, 0/90/180/270도, 숨김/표시와 원문 DOM 비복사를 확인합니다. 일반 경로는 진단 DOM 비생성과 기존 Viewer·오프라인 자산·입력·보안 회귀를 검사합니다. 실제 읽기 권한 거부는 **생성한 시험 파일 하나의 권한만 임시 변경하고 복원**합니다.
- `test:native`: 실제 패키지의 Windows 선택 창에서 한글 합성 PDF 선택·텍스트와 위치 분석 가능·키워드/영역 후보 없음·취소·원본 불변을 검사합니다. 해당 앱 소유 대화상자만 조작합니다.
- `test:shutdown`: 개발/패키지를 표시 직후·250ms 후·1500ms 후 닫는 검사를 3회 반복합니다. 최종 Unit 2.5 실행은 개발 즉시 종료 1회에서 기존 GPU 오류가 재현되어 개발 8/9·패키지 9/9입니다. 모든 창 종료·종료 코드·포트 정리는 정상입니다.
