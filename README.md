...
**Unit 3.5 정답 추출**을 구현했습니다. 사용자가 확정한 한 페이지·한 문제의 정답 영역에서만 ①부터 ⑤, 1부터 5 값을 추출하며, 단일·복수·없음·범위 초과·보기 수 불일치를 구분합니다. 답 확인 전에는 추출 값을 화면에 노출하지 않으며, 정답 비교·채점은 아직 없습니다.

버전 0.3.5의 Node 119개, 실제 PDF.js, 개발·빌드 Electron 검사는 통과했습니다. 패키지 재검증은 실행 중인 이전 앱을 종료한 뒤 진행합니다. Unit 1.0은 미착수이며 OPEN-09 GPU 종료 진단은 별도 미해결입니다. 다음 계획 Unit은 3.6 자동 채점입니다. [Project Bible](docs/PROJECT_BIBLE.md), [Roadmap](docs/ROADMAP.md), [Decisions](docs/DECISIONS.md), [Changelog](docs/CHANGELOG.md)를 확인하세요.
...
9. 정답 영역의 텍스트가 `정답: ④`처럼 단일 값이면 사이드의 `정답 추출`에 값을 확인했다는 안내만 보이고, 답 확인 전에 `4번`이 노출되지 않아야 합니다. 답 확인 뒤에만 추출된 값과 `채점은 아직 하지 않습니다.` 안내가 보여야 합니다.
10. 단일 값이 없거나 여러 개, 6번 이상, 현재 보기 수보다 큰 값이면 정답을 확정하지 않아야 하며, 맞음/틀림·채점 결과가 나타나면 안 됩니다. `npm run dev:debug`에서는 CBT 가림·수동 설정·답 선택·정답 추출 카드가 모두 숨겨져야 합니다.
...
- `npm test`: 119개. 지정 정답 영역의 단일·복수·없음·범위 초과·보기 수 불일치, 원문 문자열 비노출과 revision 불일치 차단을 더했습니다.
...
Unit 1.0의 전체 샘플 행렬은 미구현입니다. Unit 3.5는 한 페이지·한 문제, 해설·정답 사각형 각 하나와 4/5지 단일 선택·확정·공개·정답 값 추출만 지원하며 여러 문제·여러 열·여러 영역·여러 페이지 연결과 영구 저장은 지원하지 않습니다. 사용자가 직접 범위를 확인해야 하고 자동 후보는 확정을 대신하지 않습니다. 정답 추출은 확정한 정답 영역 안의 텍스트만 대상으로 하며, 값 없음·복수·범위 초과·보기 수 불일치는 보류합니다. 답 비교·맞음/틀림 표시·채점은 Unit 3.6 범위입니다. 수동 영역 재편집·파일 교체·새로고침·종료 때 상태는 사라집니다. 사각형은 포인터로만 만들 수 있어 키보드 좌표 입력·손잡이 크기 조절·터치 실기기 검증이 남았습니다. 640×480에서는 앱 main 내부 스크롤이 필요하며 실제 Windows 200% 디스플레이 배율·스크린 리더는 검증하지 않았습니다. 사용자 회전·Text Layer는 없습니다. Canvas backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한합니다. 기존 GPU 문제는 DECISIONS의 OPEN-09로 추적합니다. 이 프로젝트의 Git 저장소는 Unit 1.1 기준 커밋부터 시작했습니다.
