...
- 작성일: 2026-08-31
- 갱신일: 2026-09-01
- 상태: Unit 1.2 파일 드롭 구현·기능 검사 통과 / 기존 종료 GPU 오류로 전체 완료 보류
- 현재 산출물: 기준 문서, 한국어 Shell, 제한된 PDF 파일 선택·드롭·기본 검사, Windows x64 패키지와 반복 가능한 검사. PDF 페이지 표시·CBT 기능은 아직 없다.
- 적용 순서: 사용자의 명시적 지시 → 승인된 Project Bible → ROADMAP → DECISIONS → 구현.
- 문서의 **제안**은 사용자 요구와 구별한다. 이번 개발 승인은 사용자가 명시한 Unit 1.2에 한하며 모든 후속 Unit의 일괄 실행을 뜻하지 않는다.
...
사용자가 Unit 1.1 이후 **Unit 1.2 진행을 명시적으로 요청**했다. 이번에는 드롭을 기존 파일 선택 검사에 연결하고 다중 파일·폴더·빈 경로·URL을 거절하는 범위만 구현한다. Unit 0.4의 GPU 문제와 Unit 1.0 미착수는 완료로 바꾸지 않는다. 전체 샘플 행렬·PDF 파서·렌더링·CBT는 구현하지 않는다. 드롭 기능 검사는 통과했지만 기존 종료 회귀가 실패하여 Unit 전체의 무오류 완료 판정은 보류한다.
...
아래는 전체 목표 설계다. Unit 0.3 Shell 위에 Unit 1.1의 main 전용 파일 선택·읽기 검사와 Unit 1.2의 실제 File 기반 드롭을 추가했다. 두 입력은 같은 읽기 전용 검사를 사용하고 renderer에는 이름·크기만 반환한다. PDF 파서·렌더링·분석·CBT·데이터 저장은 아직 없다. [Electron 프로세스 모델](https://www.electronjs.org/docs/latest/tutorial/process-model).
...
Unit 0.3 Shell의 1120×760 기본 창과 640×480 최소 창을 유지한다. Unit 1.1의 상단 PDF 선택 버튼과 Unit 1.2의 작업 공간 드롭은 같은 선택/검사 중·성공·취소·실패 안내와 파일명·크기 표시를 사용한다. 취소·실패 시 이전 선택을 유지하고 성공 시 교체한다. 이름은 textContent로 표시하며 상태는 문구와 role=status로 전달한다. 시각적 drag-over 상태와 선택 버튼의 키보드 대안을 유지하지만 페이지 표시·학습 제어는 없다. 선택은 새로고침·종료 시 사라지고 기본 검사 통과를 PDF 구조 유효성으로 안내하지 않는다.
