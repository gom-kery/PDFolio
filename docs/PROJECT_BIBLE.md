...
Unit 3.1에서 일반 Viewer는 PDF를 연 뒤 `CBT 가림` 상태를 함께 표시한다. 확정이 없는 페이지와 준비 중 페이지는 Canvas 전체를 가리고, 확정된 페이지는 두 영역 Mask만 남겨 문제와 보기를 표시한다. 사용자가 `영역 설정 시작`을 누르면 전체 덮개를 숨기고 설정 Overlay를 우선하며, 확정 또는 취소 후 CBT 가림 상태로 돌아간다. 선택지·답 확인·공개·채점 버튼은 다음 Unit 전까지 만들지 않는다.
...
MVP 데이터는 필요한 Unit에서만 도입하는 메모리 레코드다. Unit 3.0은 아래 v1 계약을 확정했고 Unit 4.1은 Question v1과 Region v1의 page-single 수동 확정 범위만 구현했다. Answer v1과 Attempt v1은 아직 계약뿐이며 DB 설계나 즉시 구현할 전체 클래스 목록이 아니다.
