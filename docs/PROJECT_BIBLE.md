...
- 작성일: 2026-08-31
- 갱신일: 2026-09-01
- 상태: Unit 1.3 PDF.js 첫 페이지 렌더링 구현·기능 검사 통과 / 기존 종료 GPU 오류로 전체 완료 보류
- 현재 산출물: 기준 문서, 한국어 Shell, 제한된 PDF 선택·드롭, 로컬 PDF.js 첫 페이지 Canvas, Windows x64 패키지와 반복 가능한 검사. 페이지 이동·배율·CBT 기능은 아직 없다.
- 적용 순서: 사용자의 명시적 지시 → 승인된 Project Bible → ROADMAP → DECISIONS → 구현.
- 문서의 **제안**은 사용자 요구와 구별한다. 이번 개발 승인은 사용자가 명시한 Unit 1.3에 한하며 모든 후속 Unit의 일괄 실행을 뜻하지 않는다.
...
사용자가 **Unit 1.3 진행을 명시적으로 요청**했다. 이번에는 선택·드롭으로 승인된 PDF의 전체 바이트를 읽기 전용으로 전달하고, 로컬 PDF.js worker·자산으로 첫 페이지만 Canvas에 표시하며 암호 필요·손상 상태를 안내한다. Unit 0.4의 GPU 문제와 Unit 1.0 미착수는 완료로 바꾸지 않는다. 페이지 이동·확대·축소·Text Layer·분석·CBT는 구현하지 않는다. 렌더링 기능 검사는 통과했지만 기존 종료 회귀가 실패하여 Unit 전체의 무오류 완료 판정은 보류한다.
...
아래는 전체 목표 설계다. Unit 1.3에서는 선택·드롭이 같은 main 읽기 전용 경계를 사용하고, 승인된 PDF의 이름·크기·바이트만 renderer에 전달한다. 경로는 반환하지 않는다. PDF 어댑터가 PDF.js 객체·worker·첫 페이지 렌더·취소·정리를 소유하며 UI는 결과 상태와 Canvas만 다룬다. 분석·CBT·데이터 저장은 아직 없다. [Electron 프로세스 모델](https://www.electronjs.org/docs/latest/tutorial/process-model).
...
Unit 0.3 Shell의 1120×760 기본 창과 640×480 최소 창을 유지한다. Unit 1.1의 선택 버튼과 Unit 1.2의 드롭은 같은 읽기 경계를 사용한다. Unit 1.3은 첫 페이지만 100% viewport와 디스플레이 픽셀 비율에 맞춘 Canvas로 표시하고 전체 페이지 수를 안내한다. 파일 입력 실패·취소는 이전 성공 화면을 유지하지만, 새 파일이 암호 필요·손상으로 판정되면 이전 Canvas를 숨기고 새 오류를 표시한다. 이름은 textContent로, 상태는 문구와 role=status로 전달한다. 페이지 이동·배율·학습 제어는 만들지 않으며 선택과 PDF.js 객체는 새로고침·교체·종료 시 정리한다.
