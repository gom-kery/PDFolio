...
- 현재 산출물: Unit 2.7.2 원문 Viewer Shell과 Phase 2 분석 계약, Unit 3.0의 상태·소유 관계 계약, Unit 4.1의 한 페이지·한 문제용 수동 Question/Region 확정, Unit 3.1의 안전한 CBT Mask. 정답 추출·선택·공개·채점 UI는 아직 구현하지 않았으며 다음 계획 Unit은 3.2이다.
...
Unit 3.2는 이 `ready` 경로에만 4지/5지 설정과 하나의 답 선택을 더한다. 선택지는 native radio로 표시해 Tab과 방향키로 이동·선택하며, 사용자가 보기 수를 바꾸면 기존 선택을 지운다. 선택은 같은 Question/revision의 세션 메모리에만 두고 수동 영역 재편집, 파일 교체, 새로고침과 종료에서 폐기한다. 이 Unit은 `답 확인`·잠금·Mask 공개·정답 추출·채점을 만들지 않는다.
...
Unit 3.1에서 일반 Viewer는 PDF를 연 뒤 `CBT 가림` 상태를 함께 표시한다. 확정이 없는 페이지와 준비 중 페이지는 Canvas 전체를 가리고, 확정된 페이지는 두 영역 Mask만 남겨 문제와 보기를 표시한다. 사용자가 `영역 설정 시작`을 누르면 전체 덮개를 숨기고 설정 Overlay를 우선하며, 확정 또는 취소 후 CBT 가림 상태로 돌아간다. Unit 3.2에서는 이 준비가 끝난 경우에만 오른쪽 사이드에 `답 선택` 카드를 표시한다. 4지/5지 설정과 답 하나의 선택은 변경할 수 있지만 답 확인·공개·채점 버튼은 만들지 않는다.
...
MVP 데이터는 필요한 Unit에서만 도입하는 메모리 레코드다. Unit 3.0은 아래 v1 계약을 확정했고 Unit 4.1은 Question v1과 Region v1의 page-single 수동 확정 범위를 구현했다. Unit 3.2는 Attempt 전의 `questionId`·`documentRevision`별 보기 수와 단일 선택 상태만 세션 메모리에 구현했다. Answer v1과 Attempt v1의 잠금·공개·채점은 아직 계약뿐이며 DB 설계나 즉시 구현할 전체 클래스 목록이 아니다.
