...
- 작성일: 2026-08-31
- 갱신일: 2026-09-01
- 상태: Unit 1.1 파일 선택 구현·기능 검사 통과 / 기존 종료 GPU 오류로 전체 완료 보류
- 현재 산출물: 기준 문서, 한국어 Shell, 제한된 PDF 파일 선택·기본 검사, Windows x64 패키지와 반복 가능한 검사. PDF 페이지 표시·CBT 기능은 아직 없다.
- 적용 순서: 사용자의 명시적 지시 → 승인된 Project Bible → ROADMAP → DECISIONS → 구현.
- 문서의 **제안**은 사용자 요구와 구별한다. 이번 개발 승인은 사용자가 명시한 Unit 1.1에 한하며 모든 후속 Unit의 일괄 실행을 뜻하지 않는다.
...
사용자가 Unit 0.4의 미해결 상태를 안내받은 뒤 **Unit 1.1 진행을 명시적으로 요청**했다. 이번에는 파일 선택·기본 입력 검사·이름/크기 표시만 구현한다. Unit 0.4의 GPU 문제와 Unit 1.0 미착수는 완료로 바꾸지 않는다. 전체 샘플 행렬·드롭·PDF 파서·렌더링·CBT는 구현하지 않는다. 파일 선택 기능 검사는 통과했지만 기존 종료 회귀가 실패하여 Unit 전체의 무오류 완료 판정은 보류한다.
...
PDF.js는 아직 설치하지 않았으며 렌더링을 시작하는 Unit에서 호환 버전을 선택한다. 실행·설치 방법은 [README](../README.md), 버전·모듈 형식과 환경 검증의 결정 이유는 DECISIONS의 ADR-011~016을 따른다.

## 4. 전체 아키텍처

아래는 전체 목표 설계다. Unit 0.3 Shell 위에 Unit 1.1의 main 전용 파일 선택·읽기 검사와 좁은 preload API를 추가했다. renderer에는 이름·크기만 반환한다. PDF 파서·렌더링·분석·CBT·데이터 저장은 아직 없다. [Electron 프로세스 모델](https://www.electronjs.org/docs/latest/tutorial/process-model).

```mermaid
flowchart LR
  LocalPDF[사용자가 선택한 로컬 PDF] --> Main[Main: 읽기 전용 파일 접근]
...
| 경계 | 책임 | 하지 않는 일 |
| --- | --- | --- |
| Main | 창 수명, 파일 선택·검증·읽기, IPC 검증. 영구 저장은 이후 추가 | 문제 인식, 채점, PDF 스크립트 실행 |
...
**Unit 1.1에서 사용하는 주요 구조:**

```text
local-pdf-cbt/
├─ docs/                기준 문서 5개
├─ electron/            main·preload·로컬 프로토콜·PDF 선택/검사
├─ src/main.js          화면 초기화
├─ src/ui/              실행 환경·파일 선택 상태 표시
├─ src/styles/          기본 스타일·Shell 배치
├─ scripts/             Node 검사·개발 실행·패키징
├─ tests/               보안 경계·실제 Electron 환경 검사
├─ index.html
├─ vite.config.js
├─ package.json
├─ package-lock.json
...
Unit 0.3 Shell의 1120×760 기본 창과 640×480 최소 창을 유지한다. Unit 1.1은 상단 PDF 선택 버튼, 선택/검사 중·성공·취소·실패 안내, 파일명·크기 표시를 추가한다. 취소·실패 시 이전 선택을 유지하고 성공 시 교체한다. 이름은 textContent로 표시하며 상태는 문구와 role=status로 전달한다. 페이지 표시·드롭·학습 제어는 없다. 선택은 새로고침·종료 시 사라지고 기본 검사 통과를 PDF 구조 유효성으로 안내하지 않는다.
...
이번 순서 조정은 사용자의 명시적 Unit 1.1 요청에 한한다. 미착수 Unit 1.0을 완료로 바꾸거나 기존 오류 없음 조건을 낮추지 않는다. 필요한 최소 입력 상한·서명 정책만 ADR-017/018에 기록한다.
