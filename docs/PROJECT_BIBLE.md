...
| PDF 열람 | 파일 선택, 파일 드롭, 렌더링, 페이지 이동, 확대·축소, 화면 맞춤 |
...
### Unit 3.6.5 완료 — Viewer 작업 공간과 화면 맞춤

페이지 이동과 배율 제어를 오른쪽 사이드바에서 제거하고 문서 작업 공간의 상단 도구모음으로 옮겼다. 문서 정보와 현재 페이지 분석은 기본 닫힘 아코디언이며, CBT 가림·정답 추출·채점 결과는 사이드바에서 계속 확인할 수 있다. `높이 맞춤`은 `화면 맞춤`으로 바꾸고, 가용 너비와 높이를 모두 넘지 않는 최대 배율(`contain`)을 50~200% 범위에서 계산한다. 공개 뒤의 현재 Question 결과는 원본 PDF 위에 쓰지 않고 작업 공간 제목 옆의 짧은 배지로도 표시한다. 이 배지는 기존 Grade v1 결과만 다시 표시하며 여러 문제 페이지의 문제별 식별·표시는 하지 않는다. 임시 Canvas 완료 후 교체, Mask 투영, 공개·선택·추출·채점 세션 상태 불변 계약은 유지한다.

### Unit 3.7 완료 — 단일 문제 첫 MVP 통합 검증

A형은 `해설→정답` 순서의 4지 문제를 4번 선택해 정답으로, B형은 `정답→해설` 순서의 5지 문제를 3번 선택해 오답으로 끝까지 검증했다. 정답을 하나로 확정하지 못하는 기존 흐름은 해설·정답 공개 뒤에도 `채점 불가`로 남는지 함께 확인했다. 지원 외·분석 중 입력의 전체 가림, 선택·공개·결과의 페이지 탐색·배율 변경 유지와 파일 교체 초기화, Canvas 전환, Text Layer 부재, 로컬 worker·글꼴·CMap 자산과 외부 요청 차단, 원본 해시 불변을 개발·빌드·Windows x64 ASAR 패키지에서 검증했다. 이는 한 페이지·한 문제 첫 MVP의 완료 기록이며, 여러 문제가 있는 한 페이지를 각각 가리고 풀 수 있는 넓은 MVP는 Phase 4의 4.0·4.2·4.3·4.4·4.5 완료 뒤에만 완료로 판단한다.
...
아래는 전체 목표 설계다. 선택·드롭은 같은 main 읽기 전용 경계를 사용하고 승인된 PDF의 이름·크기·바이트만 renderer에 전달하며 경로는 반환하지 않는다. PDF 어댑터는 PDF.js document/page/TextContent 객체와 worker·렌더·추출 수명을 소유한다. 분석 모듈에는 버전 있는 순수 데이터 레코드만 전달하고, Unit 2.3의 분석 모듈은 페이지 텍스트 품질, PDF user space bbox, 제목 키워드 후보까지 만든다. UI는 어댑터나 분석 모듈의 원시 객체·추출 원문·자동 후보 문자열을 보관하지 않고 공개 상태 요약만 표시한다. Unit 4.1의 수동 Question/Region과 Unit 3.1의 현재 viewport Mask만 세션 메모리에 있으며 Answer·Attempt와 영구 저장은 아직 없다. [Electron 프로세스 모델](https://www.electronjs.org/docs/latest/tutorial/process-model).
...
├─ src/cbt/             수동 page-single Question/Region 설정·검증·세션 확정·Mask 준비 판정
├─ src/shared/          PageTextSource v1 검증과 계약 버전
├─ src/ui/              실행 환경·Viewer·분석 상태·수동 영역 설정·CBT Mask 화면
...
13. Unit 4.1 완료: 텍스트 분석 품질과 무관하게 현재 PDF 페이지에서 해설·정답 사각형을 각각 하나 드래그하고 불투명 미리보기 뒤 원자적으로 Question/Region을 확정한다. PDF user space 좌표의 확대·높이 맞춤·0/90/180/270도 왕복, 겹침·범위·최소 크기 거부, 취소·재편집·페이지 이동·파일 교체 무효화를 Node 108개와 Electron 4경로에서 검증했다. 설정 화면은 원문 노출 가능성을 알리며 CBT Mask로 사용하지 않는다.
14. Unit 3.1 완료: 같은 revision·페이지의 확정 Question/Region을 다시 검증한 뒤 해설·정답만 불투명하게 가린다. 미확정·편집·stale·render 준비 중에는 전체 덮개를 유지하고 설정 화면에서만 원문을 보인다. Node 111개와 Electron 4경로에서 전체 덮개·확정 Mask·viewport 재투영·Text Layer 부재·Canvas 접근성 차단을 검증했다.
...
### 9.9 Unit 3.0 계약과 Unit 4.1 수동 영역 확정

Unit 3.0은 문서 전용 구조 검토였고 Unit 4.1은 그 선행 관문의 첫 MVP를 구현했다. Phase 2의 `PageAnswerRegions`와 `PageSupportProfile`은 계속 승인 전 후보이며 이를 `Question`이나 `Region`으로 자동 승격하지 않는다. 사용자가 직접 지정·미리보기·확정한 현재 페이지의 두 Region만 Question에 속한다. Unit 3.1은 이 확정 Region으로 실제 CBT Mask를 표시한다. 확정 또는 렌더 준비가 없을 때는 전체 덮개를 유지하고, 원문은 사용자가 시작한 설정 화면에서만 보인다.
...
5. 페이지 이동은 미확정 편집만 취소하고 같은 세션·페이지로 돌아오면 확정 상태를 유지한다. 파일 교체·revision 변경·새로고침·종료는 모든 수동 확정을 폐기한다.
6. Unit 4.1은 위 기능과 좌표 왕복 검증을 완료했다. Unit 3.1은 이 확정 레코드를 입력으로 별도 Mask Layer와 준비 전 전체 덮개를 구현했다. 영구 저장은 Phase 5까지 하지 않는다.

Unit 4.1 설정 UI는 일반 실행에서만 사용한다. 명시적 Debug Overlay 실행에서는 두 진단 목적을 동시에 켜지 않고 수동 설정을 숨긴다. 포인터 드래그는 Canvas 표시 사각형을 PDF user space로 역변환하고 6자리 소수로 정규화한다. 8 CSS px 미만 드래그, 페이지 밖·0 크기·비유한 좌표와 두 종류의 실제 겹침은 확정하지 않는다. 미리보기는 두 영역을 불투명하게 덮지만 확인용이며 완료 뒤 숨는다. 확정·취소·재편집은 원본 PDF 바이트를 바꾸지 않는다.

Unit 3.1 Mask는 동일 `documentRevision`·`pageNumber`의 `confirmed` Question과 Region만 다시 검증해 사용한다. contract version·source·questionId/regionId·두 종류·PDF user space·페이지 범위·비겹침이 맞지 않으면 추측하지 않고 전체 덮개를 보인다. 렌더 결과가 확정된 뒤에는 해설·정답 사각형만 불투명하게 다시 투영한다. 확대·높이 맞춤·고유 회전·페이지 이동·파일 교체·수동 재편집에서 새 화면과 새 Mask가 모두 준비되기 전에는 기존 영역을 재사용하지 않는다. 현 Viewer는 Text Layer를 만들지 않으며 가림 상태에서는 Canvas를 접근성 트리에서 숨긴다. 설정 화면에서만 Canvas 접근성 이름을 되돌리며, 이는 DRM이나 완전한 스크린 리더 지원이 아니다.

Unit 3.2는 이 `ready` 경로에만 4지/5지 설정과 하나의 답 선택을 더한다. 선택지는 native radio로 표시해 Tab과 방향키로 이동·선택하며, 사용자가 보기 수를 바꾸면 기존 선택을 지운다. 선택은 같은 Question/revision의 세션 메모리에만 둔다. Unit 3.3은 선택 전 확인을 UI와 도메인에서 차단하고, 선택 후 `답 확인`을 누르면 `selected → locked`로 한 번만 전이한다. Unit 3.4는 이 잠긴 선택과 현재 Mask의 Question/revision이 일치할 때만 해설·정답 두 Mask를 공개한다. Unit 3.5는 같은 revision·페이지의 확정 정답 영역에서 나온 단일 `known` 값만 이용한다. Unit 3.6은 공개 성공 뒤 한 번만 비교하여 맞음·틀림·채점 불가 결과를 남긴다. 공개와 결과는 같은 Question/revision의 세션 메모리에만 두고 재방문 때 복원하며, 다른 페이지·Question으로 전파하지 않는다. 수동 영역 재편집·파일 교체·새로고침·종료에서는 선택·공개·결과를 폐기한다.
...
Unit 3.6.5는 위의 Viewer Shell에서 현재 페이지·배율 도구를 문서 작업 공간 상단 도구모음으로 옮긴다. `문서 정보`와 `현재 페이지 분석`은 기본 닫힘 아코디언으로 바꾸고, CBT 가림·정답 추출·채점 결과는 계속 아코디언 밖에 둔다. `화면 맞춤`은 scale 1 viewport의 가로·세로와 PDF 스크롤 영역의 가용 너비·높이를 모두 비교해 더 작은 배율을 50–200%로 제한한다. 화면 맞춤 상태의 resize는 새 목표와 현재 배율 차이가 0.001보다 클 때만 임시 Canvas 렌더를 요청한다. Grade v1 결과가 공개되면 원본을 바꾸지 않는 작업 공간 제목 옆 배지에 `정답입니다`·`오답입니다`·`채점 불가`를 간단히 표시하고, 수동 재편집·파일 교체 등 기존 무효화와 함께 숨긴다.

Unit 4.1의 `해설·정답 영역 설정` 카드는 PDF를 연 일반 실행의 Viewer 사이드에 표시한다. 시작하면 원문과 정답이 보일 수 있고 아직 CBT가 아니라는 경고를 유지한다. 사용자는 해설·정답 종류를 고르고 PDF 위에서 각각 한 번씩 드래그하며, 두 영역이 준비된 뒤에만 불투명 가림 미리보기로 들어갈 수 있다. 미리보기에서 수정·확정·취소를 명시적으로 선택한다. 확정 뒤에는 설정 Overlay를 숨겨 원문 Viewer로 돌아간다. 이미 확정한 페이지를 다시 편집하는 동안 그 확정은 Mask 입력으로 제공하지 않으며 취소하면 이전 확정을 복원하고 재확정하면 새 ID로 교체한다. 페이지 이동은 미완료 초안을 취소하고 파일 교체는 모든 확정을 폐기한다. 키보드로 버튼·상태에는 접근할 수 있지만 사각형 자체의 키보드 지정·크기 조절은 후속 접근성 과제로 남긴다.

Unit 3.1에서 일반 Viewer는 PDF를 연 뒤 `CBT 가림` 상태를 함께 표시한다. 확정이 없는 페이지와 준비 중 페이지는 Canvas 전체를 가리고, 확정된 페이지는 두 영역 Mask만 남겨 문제와 보기를 표시한다. 사용자가 `영역 설정 시작`을 누르면 전체 덮개를 숨기고 설정 Overlay를 우선하며, 확정 또는 취소 후 CBT 가림 상태로 돌아간다. Unit 3.2에서는 이 준비가 끝난 경우에만 오른쪽 사이드에 `답 선택` 카드를 표시한다. Unit 3.3은 답 하나를 고른 뒤에만 `답 확인`을 활성화하고, 확인하면 선택·보기 수·확인 버튼을 잠근다. Unit 3.4는 같은 Question의 해설·정답 Mask를 숨겨 원문을 공개하고, 재방문 시에도 그 상태를 복원한다. Unit 3.5는 답 확인 전에는 정답 값을 숨기고, Unit 3.6은 공개 뒤 `맞음`·`오답` 또는 `채점 불가`만 표시한다. 정답이 없거나 복수·범위 밖·분석 대기면 틀림으로 표시하지 않는다.
...
MVP 데이터는 필요한 Unit에서만 도입하는 메모리 레코드다. Unit 3.0은 아래 v1 계약을 확정했고 Unit 4.1은 Question v1과 Region v1의 page-single 수동 확정 범위를 구현했다. Unit 3.2·3.3은 `questionId`·`documentRevision`별 보기 수·단일 선택·잠금을, Unit 3.4는 공개를, Unit 3.5는 추출 Answer를, Unit 3.6은 결과를 세션 메모리에 구현했다. 이는 영구 Attempt 이력이나 DB 설계가 아니다.
...
Unit 4.0은 아래의 **후속 확장 계약**을 채택했다. 이는 실행 중인 v1 레코드를 변경하거나 저장소를 추가하는 명세다. Unit 4.2는 이를 바꾸지 않는 `PageQuestionCandidates v1` 초안 분석만 구현했고, Unit 4.3은 기존 v1을 바꾸지 않는 세션 연결 레코드와 검증만 추가했다.

| 확장 개념 | 후속 계약과 안전 규칙 |
| --- | --- |
| Question v2 | `questionId`는 세션 독립 ID로 유지하고 `documentRevision`·`sourceKind`·`setupStatus`를 가진다. `pageRefs[]`는 단순 페이지 번호 배열이 아니라 `pageNumber`와 `role: prompt/choice/explanation/answer`를 가진 순서 있는 참조다. 페이지 번호·인쇄 문제 번호·후보 순서는 ID나 영구 순서가 아니다. |
| Region v2 | 기하 정보는 한 페이지의 PDF user space 사각형으로 유지하되 `regionId`는 Question 소유가 아니다. Question과의 연결은 별도 Binding이 담당한다. 다페이지 영역은 사각형 하나를 늘리는 대신 여러 Region으로 표현한다. |
...
순서 조정은 사용자가 명시적으로 요청한 Unit에만 적용한다. Unit 2.3과 2.4를 Unit 2.5보다 먼저 구현한 예외와 뒤이은 Unit 2.5 완료는 기존 오류 없음 조건을 낮추지 않는다. 최소 입력 상한·서명 정책은 ADR-017/018, 드롭 경계는 ADR-019, Phase 2 순서 예외와 분석 계약은 ADR-027/028, 진단 Overlay 경계는 ADR-029, Phase 3 전 Viewer Shell 경계는 ADR-031, Unit 3.0 CBT 상태·소유 관계와 Unit 4.1 선행 결정은 ADR-033, Unit 4.1 구현 경계는 ADR-034, Unit 3.1 Mask 경계는 ADR-035에 기록한다.
