...
- 현재 산출물: Unit 2.7.2 원문 Viewer Shell과 Phase 2 분석 계약, Unit 3.0의 상태·소유 관계 계약, Unit 4.1의 한 페이지·한 문제용 수동 Question/Region 확정, Unit 3.1의 안전한 CBT Mask, Unit 3.2의 4/5지 단일 선택, Unit 3.3의 답 선택 확정·잠금. 해설·정답 공개·정답 추출·채점 UI는 아직 구현하지 않았으며 다음 계획 Unit은 3.4이다.
...
MVP 데이터는 필요한 Unit에서만 도입하는 메모리 레코드다. Unit 3.0은 아래 v1 계약을 확정했고 Unit 4.1은 Question v1과 Region v1의 page-single 수동 확정 범위를 구현했다. Unit 3.2는 Attempt 전의 `questionId`·`documentRevision`별 보기 수와 단일 선택 상태를, Unit 3.3은 `unselected / selected / locked`의 확인 전이만 세션 메모리에 구현했다. 이는 Attempt v1 생성이나 정답·공개·채점 결과가 아니며 DB 설계나 즉시 구현할 전체 클래스 목록이 아니다.
