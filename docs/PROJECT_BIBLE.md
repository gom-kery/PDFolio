...
Unit 3.2는 이 `ready` 경로에만 4지/5지 설정과 하나의 답 선택을 더한다. 선택지는 native radio로 표시해 Tab과 방향키로 이동·선택하며, 사용자가 보기 수를 바꾸면 기존 선택을 지운다. 선택은 같은 Question/revision의 세션 메모리에만 둔다. Unit 3.3은 선택 전 확인을 UI와 도메인에서 차단하고, 선택 후 `답 확인`을 누르면 `selected → locked`로 한 번만 전이한다. Unit 3.4는 이 잠긴 선택과 현재 Mask의 Question/revision이 일치할 때만 해설·정답 두 Mask를 공개한다. 공개는 같은 Question/revision의 세션 메모리에만 두고 재방문 때 복원하며, 다른 페이지·Question으로 전파하지 않는다. 수동 영역 재편집·파일 교체·새로고침·종료에서는 선택과 공개를 폐기한다. 정답 추출·채점은 만들지 않는다.
