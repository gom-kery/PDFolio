...
- 작성일: 2026-08-31
- 갱신일: 2026-09-01
- 상태: Unit 1.5 PDF 확대·축소·너비 맞춤 구현·기능 검증 통과 / 최종 종료 검사 15/18, OPEN-09 재현으로 전체 완료 보류
- 현재 산출물: 기준 문서, 한국어 Shell, 제한된 PDF 선택·드롭, 로컬 PDF.js 현재 페이지 Canvas·페이지 이동·50–200% 배율·너비 맞춤, Windows x64 패키지와 반복 가능한 검사. Text Layer·분석·CBT 기능은 아직 없다.
- 적용 순서: 사용자의 명시적 지시 → 승인된 Project Bible → ROADMAP → DECISIONS → 구현.
- 문서의 **제안**은 사용자 요구와 구별한다. 이번 개발 승인은 사용자가 명시한 Unit 1.5와 함께 채택한 Viewer 배치 변경에 한하며 모든 후속 Unit의 일괄 실행을 뜻하지 않는다.
...
사용자가 **Unit 1.5 진행과 Viewer 배치 아이디어 포함을 명시적으로 요청**했다. Unit 1.4의 현재 페이지 계약 안에서 50–200% 단계 배율, 너비 맞춤, 고유 회전·디스플레이 픽셀 비율·창 크기 대응, 이전 렌더 취소와 Canvas 자원 상한을 제공한다. 페이지·배율 도구는 PDF 아래, 이전·다음 보조 버튼은 PDF 좌우에 두며 작은 창에서는 하단 이동 도구를 유지한다. 중복된 렌더 완료 행을 없애고 원본 불변 성공 안내는 footer로 옮긴다. Unit 0.4의 과거 간헐적 GPU 문제와 Unit 1.0 미착수는 완료로 바꾸지 않는다. 사용자 회전 버튼·Text Layer·분석·CBT·저장은 구현하지 않는다. Unit 1.5 기능 검사는 모두 통과했지만 최종 종료 반복에서 OPEN-09가 개발 모드 즉시 종료 3회에 재현되어 전체 무오류 완료 판정은 보류한다.
...
아래는 전체 목표 설계다. Unit 1.5에서는 선택·드롭이 같은 main 읽기 전용 경계를 사용하고, 승인된 PDF의 이름·크기·바이트만 renderer에 전달한다. 경로는 반환하지 않는다. PDF 어댑터가 PDF.js 객체·worker·현재 한 페이지 렌더·배율·고유 회전·backing pixel 상한·요청 식별자·취소·정리를 소유하며 UI는 1부터 시작하는 페이지 번호, CSS 배율과 Canvas 상태만 다룬다. 분석·CBT·데이터 저장은 아직 없다. [Electron 프로세스 모델](https://www.electronjs.org/docs/latest/tutorial/process-model).
...
Unit 0.3 Shell의 1120×760 기본 창과 640×480 최소 창을 유지한다. Unit 1.1의 선택 버튼과 Unit 1.2의 드롭은 같은 읽기 경계를 사용한다. Unit 1.4의 현재 한 페이지·1부터 시작하는 번호·최신 렌더 우선 계약을 유지하면서 Unit 1.5는 50·75·100·125·150·175·200% 단계 배율과 페이지별 너비 맞춤을 제공한다. 너비 맞춤은 PDF 스크롤 영역의 실제 CSS 폭을 사용하고 창 크기가 바뀌면 다시 계산한다. PDF 고유 회전은 PDF.js viewport로 반영하되 사용자 회전 버튼은 만들지 않는다. Canvas CSS 크기와 backing pixel 배율을 분리하고, backing bitmap은 최대 16,777,216픽셀·한 변 8,192픽셀로 제한한다. 제한 시 화면 배율은 유지하고 렌더 해상도만 낮춘다. 페이지·배율 도구는 PDF 아래, 같은 상태를 공유하는 이전·다음 보조 버튼은 PDF 좌우에 두며 40rem 이하에서는 보조 버튼을 숨긴다. 정상 렌더 완료 문구의 중복 행은 숨기고 `PDF를 열었습니다. 원본 파일은 변경하지 않았습니다.`를 footer의 `role=status`에 표시한다. 로딩·오류·잘못된 번호 안내는 Viewer 상단에 유지한다. 선택과 PDF.js 객체는 새로고침·교체·종료 시 정리한다.
