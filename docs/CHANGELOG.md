...
- 현재 `Question/Region/Answer/Attempt v1`을 변경하지 않는 후속 확장 계약을 정했다. Question의 페이지 참조와 Region의 기하 정보를 분리하고, 후속 `QuestionRegionBinding`으로 관계를 표현한다.
- 해설·정답은 한 Question에만 속하는 `question-only` Region으로 제한했다. 여러 Question이 공유할 수 있는 것은 정답을 포함하지 않는 지문·공통 설명뿐이며, 경계가 불명확하면 CBT를 시작하지 않는다.
- 자동 분리 후보는 draft만 만들고 확정된 수동 보정을 덮어쓰지 않는다. 자동 후보·수동 확정의 충돌은 보류하고 사용자가 병합·분할·연결 해제하기 전까지 공개하지 않는다.
- Question 순서는 페이지 번호나 인쇄 번호에서 추론하지 않고, 후속 Unit 4.4의 별도 확정 관계로만 도입하기로 했다.

### Unit 4.0 — 2. 수정된 파일

| 구분 | 파일·변경 |
| --- | --- |
| 제품 기준 | `README.md` — 현재 위치와 다음 Unit 갱신 |
| 확장 계약 | `docs/PROJECT_BIBLE.md` — v2 관계·공개·공유·무효화 원칙 |
| 일정 | `docs/ROADMAP.md` — 4.0 완료·4.2 착수 조건 |
| 결정 | `docs/DECISIONS.md` — ADR-043 |
| 완료 기록 | `docs/CHANGELOG.md` — Unit 4.0 문서 전용 기록 |

### Unit 4.0 — 3. 실행 방법

문서 전용 Unit입니다. 앱 실행·빌드·패키지 생성은 하지 않았습니다. 기준 문서는 `docs/PROJECT_BIBLE.md`, `docs/ROADMAP.md`, `docs/DECISIONS.md` 순서로 읽습니다.

### Unit 4.0 — 4. 직접 확인할 방법

1. `docs/ROADMAP.md`의 Phase 4 표에서 Unit 4.0이 완료이고 다음 Unit이 4.2인지 확인합니다.
2. `docs/PROJECT_BIBLE.md`의 `후속 확장 계약`에서 정답·해설이 `question-only`이고, 공유는 정답 없는 문맥에만 허용되는지 확인합니다.
3. `docs/DECISIONS.md`의 ADR-043에서 자동 후보가 확정 수동 보정을 덮어쓰지 않고 충돌 시 보류하는지 확인합니다.
4. 기존 패키지 앱을 열어도 한 페이지·한 문제의 기존 동작만 보이고, 문제 분리·문제 탐색 UI가 새로 생기지 않았는지 확인합니다.

### Unit 4.0 — 5. 정상 동작 기준

- 현재 v1 런타임 계약과 앱 버전 0.3.6을 바꾸지 않는다.
- 다문제 지원 전에 Question·Region 관계, 공유 금지 영역, 수동 보정 우선순위와 무효화 범위가 서로 모순 없이 문서화된다.
- 다음 Unit 4.2가 자동 분리만 다루고 다페이지 연결·문제 탐색을 함께 추가하지 않는 경계가 명확하다.

### Unit 4.0 — 6. 예상되는 Edge Case

- 공통 지문 안에 해설 또는 정답이 섞여 있으면 `shared-context`로 승인하지 않고 보류한다.
- 자동 후보가 수동 영역과 일부만 겹쳐도 자동으로 병합하거나 기존 수동 결과를 옮기지 않는다.
- 다페이지 문제의 해설·정답이 다음 페이지에 있어도 Unit 4.2에서 임의 연결하지 않는다.
- 페이지 번호·문제 번호가 중복되거나 순서가 바뀌어도 Question ID와 문제 탐색 순서로 사용하지 않는다.

### Unit 4.0 — 7. 알려진 제한사항

- 한 페이지 여러 문제·다단 자동 분리, 다페이지 연결, 문제 단위 탐색과 다문제 통합 검증은 아직 구현하지 않았다.
- Question v2·Region v2·Binding은 후속 구현 계약이며 런타임 데이터나 저장 형식이 아니다.
- Unit 1.0 실물 샘플 행렬과 OPEN-09 GPU 종료 진단은 계속 미해결이다.

### Unit 4.0 — 8. Technical Debt

- 실제 다문제 PDF로 자동 후보의 누락·중복·겹침·수동 충돌 비율을 측정해야 한다.
- 공유 문맥에 답 정보가 없는지 자동으로 판정하는 규칙은 아직 없다. Unit 4.2는 보수적 보류부터 시작한다.
- 다페이지 연결, QuestionOrder, 영구 저장은 독립 Unit에서 별도 검증해야 한다.

### Unit 4.0 — 9. 다음 Unit 진행 전 수정이 필요한 사항

필수 코드 수정은 없다. Unit 4.2는 ADR-043의 draft-only 자동 후보와 수동 보정 불변 규칙을 먼저 구현·검증해야 하며, 다페이지 연결·문제 탐색·저장을 범위에 넣으면 안 된다.

### Unit 4.0 — 10. Git Commit Message

제안: `PDFolio repository at unit 4.0_define multi-question expansion contracts`

## 0.3.6 / Unit 3.7 작업 기록 — 2026-09-09

**한 페이지·한 문제 첫 MVP의 전체 풀이 흐름과 Windows x64 ASAR 패키지를 검증했다. 여러 문제가 있는 페이지까지 포함한 넓은 MVP 완료 기준은 Phase 4 완료로 기록했다.**

### Unit 3.7 — 1. 구현·검증한 내용

- A형 4지(`해설→정답`, 정답 4번)와 B형 5지(`정답→해설`, 정답 5번) 고정 합성 PDF를 추가했다. 수동 영역 확정부터 가림·선택·잠금·공개·정답 추출·정답/오답 결과까지 한 흐름으로 자동 검사한다.
- 기존 정답 미확정 PDF는 해설·정답을 공개해도 `채점 불가`가 유지되는지, 지원 외·분석 중 입력은 전체 가림이 유지되는지 함께 회귀했다.
- 이전 3.6.5 패키지를 `work/unit-3.7-previous-package/`에 보관하고 새 Windows x64 ASAR 패키지를 만들었다. 개발·빌드·패키지 모두에서 로컬 PDF.js 자산과 외부 요청 차단을 다시 확인했다.
- 실제 Windows 파일 선택 창 검사를 현재의 접힌 `현재 페이지 분석` 구조에 맞췄다. 알려진 OPEN-09 GPU 종료 진단만 증거에 남기고, 다른 런타임·Console 오류는 계속 실패로 처리한다.

### Unit 3.7 — 2. 수정된 파일

| 구분 | 파일·변경 |
| --- | --- |
| 합성 입력·통합 검사 | `tests/helpers/pdf-fixtures.js`, `tests/helpers/pdf-selection-checks.js` — A형 4지 정답과 B형 5지 오답의 전체 CBT 흐름 |
| 실제 선택 창 검사 | `tests/native-dialog.test.js` — 접힌 분석 영역의 상태 확인과 OPEN-09 기록 정책 |
| 문서 | `README.md`, `docs/PROJECT_BIBLE.md`, `docs/ROADMAP.md`, `docs/DECISIONS.md`, `docs/CHANGELOG.md` — 3.7 완료와 Phase 4 넓은 MVP 기준 |

### Unit 3.7 — 3. 직접 확인할 방법

1. `release/local-pdf-cbt-win32-x64/local-pdf-cbt.exe`를 실행합니다.
2. 한 페이지·한 문제 PDF를 열어 `영역 설정 시작`에서 해설·정답을 각각 지정하고 확정합니다. 두 영역만 가려져야 합니다.
3. 정답 영역에 `Answer: 4` 또는 `정답: ④`처럼 단일 값이 있는 PDF에서 4번을 선택해 `답 확인`을 누릅니다. 해설·정답 공개 뒤 `정답입니다`가 보여야 합니다.
4. 새 PDF 또는 새 확정 세션에서 다른 번호를 선택하면 `오답입니다`와 정답 값이 보여야 합니다. 정답 값이 없거나 여러 개면 공개는 되지만 `채점 불가`여야 합니다.
5. 5지로 바꾸면 기존 선택이 지워지고 1~5번만 보여야 합니다. 페이지·배율을 바꿔도 같은 Question의 상태가 유지되며, `확정 영역 수정` 시작이나 PDF 교체 뒤에는 초기화되어야 합니다.
6. 여러 문제가 있는 한 페이지는 현재 범위 밖입니다. 이 경우 문제별 가림·선택·결과를 기대하지 마세요. 해당 범위는 Phase 4에서 구현합니다.
...
| --- | --- | --- |
| `npm run format:check` | 통과 | 프로젝트 형식 |
| `npm test` | 122/122 통과 | 선택·공개·추출·채점과 기존 경계 계약 |
| `npm run build` | 통과 | Vite 33 modules, 기존 500 kB 초과 번들 안내만 표시 |
| `npm run package` | 통과 | 새 Windows x64 ASAR 패키지 생성 |
| `npm run test:electron` | 4/4 통과 | Debug·개발·빌드·패키지, A형 4지 정답·B형 5지 오답·가림·오프라인·보안 |
| `npm run test:native` | 1/1 통과 | 실제 Windows 선택 창, 취소·원본 불변·접힌 분석 상태 |
| `npm run test:shutdown` | 18/18 통과 | 개발·패키지 각각 3회, 표시 직후·250ms·1.5초 뒤 창 닫기, 종료 코드 0·포트 해제 |
| `git diff --check` | 통과 | 공백 오류 없음 |

### Unit 3.7 — 5. 알려진 제한사항

- 이 완료는 한 페이지·한 문제용 첫 MVP다. 한 페이지 여러 문제·다단·다페이지 연결·문제별 탐색은 구현하지 않았다.
- 사용자가 정의한 넓은 MVP 완료 기준은 Phase 4의 4.0, 4.2, 4.3, 4.4, 4.5 완료다.
- Unit 1.0 대표 실물 PDF 행렬과 OPEN-09 GPU 종료 진단은 미해결이다. OPEN-09는 실제 창 종료·종료 코드·포트 해제가 정상인 기존 진단이며 수정은 별도 범위다.

### Unit 3.7 — 6. Git Commit Message

제안: `PDFolio repository at unit 3.7_validate single-question MVP package`

## 0.3.6 / Unit 3.6.5 작업 기록 — 2026-09-09

**페이지·배율 제어를 문서 작업 공간 상단으로 옮기고, 화면 맞춤과 현재 Question 결과 배지를 추가했다. 기존 가림·선택·공개·정답 추출·채점 계약은 바꾸지 않았다.**

### Unit 3.6.5 — 1. 구현한 내용

- 오른쪽 사이드바의 페이지 이동·배율 제어를 문서 작업 공간 상단 도구모음으로 이동했다. 좌우 페이지 이동 버튼과 키보드·입력 이동은 유지한다.
- `문서 정보`와 `현재 페이지 분석`을 기본 닫힘 아코디언으로 변경했다. CBT 가림·정답 추출·채점 결과는 계속 사이드바에서 보인다.
- `높이 맞춤`을 `화면 맞춤`으로 바꿨다. PDF 표시 영역의 가용 너비와 높이 중 더 작은 배율을 선택하고 기존 50~200% 경계를 적용한다.
- 공개된 Grade v1 결과를 문서 작업 공간 제목 옆에도 간단히 표시한다. `정답`, `오답`, `채점 불가`만 표시하며 PDF 원문·좌표·문제 텍스트는 추가로 노출하지 않는다.
- 수동 영역 편집·파일 교체·새로고침 등 기존 무효화 시 배지도 함께 숨긴다. 여러 문제 페이지의 문제별 결과 표시는 추가하지 않았다.

### Unit 3.6.5 — 2. 수정된 파일

| 구분 | 파일·변경 |
| --- | --- |
| 작업 공간 | `index.html`, `src/styles/shell.css` — 상단 도구모음, 아코디언, 결과 배지와 반응형 배치 |
| 화면 맞춤·채점 표시 | `src/ui/pdf-viewer.js`, `src/ui/grade-status.js` — contain 배율과 Grade v1 배지 동기화 |
| 검사 | `tests/electron.test.js`, `tests/debug-overlay.test.js`, `tests/native-dialog.test.js`, `tests/helpers/pdf-selection-checks.js` — 아코디언·작업 공간 도구모음·화면 맞춤·배지 무효화 회귀 |
| 문서 | `README.md`, `docs/PROJECT_BIBLE.md`, `docs/ROADMAP.md`, `docs/DECISIONS.md`, `docs/CHANGELOG.md` — Unit 완료 기록 |

### Unit 3.6.5 — 3. 직접 확인할 방법

1. `npm run dev`로 PDF를 열면 페이지 이동·확대·축소·`화면 맞춤`이 문서 작업 공간 상단에 있어야 합니다.
2. 오른쪽 `문서 정보`, `현재 페이지 분석`을 키보드 Tab과 Enter로 접고 펼칩니다. 아래의 `CBT 가림`, `정답 추출`, `채점 결과`는 아코디언 밖에 남아야 합니다.
3. `화면 맞춤`을 누르고 창을 넓히거나 줄입니다. PDF가 표시 영역의 너비와 높이를 모두 넘지 않아야 하며, 이후 `+` 또는 `−`를 누르면 고정 배율로 전환되어야 합니다.
4. 한 페이지·한 문제 PDF에서 영역을 확정하고 답을 확인합니다. 채점 결과가 생기면 작업 공간 제목 옆에도 `정답`, `오답` 또는 `채점 불가` 배지가 보여야 합니다. `확정 영역 수정`을 시작하거나 PDF를 바꾸면 배지는 사라져야 합니다.
5. 여러 문제가 있는 페이지에서는 각 문제 위에 결과를 붙이지 않습니다. 이 동작은 아직 지원 범위 밖입니다.
...
| --- | --- | --- |
| `npm run format:check` | 통과 | 프로젝트 형식 |
| `npm test` | 122/122 통과 | 기존 채점·가림·정답 추출 계약 |
| `npm run build` | 통과 | Vite 33 modules, 기존 500 kB 초과 번들 안내만 표시 |
| `npm run test:electron` | Debug·개발·빌드 3/4 통과 | 아코디언·작업 공간 도구모음·화면 맞춤 resize·Mask·결과 배지 무효화 |
| `git diff --check` | 통과 | 공백 오류 없음 |

현재 패키지 경로는 이전 앱이라 새 `현재 페이지 분석` 아코디언 DOM이 없어 4번째 패키지 경로가 실패한다. 소스 변경이 아닌 재패키지 대기 상태이며, 앱을 닫은 뒤 `npm run package`와 `npm run test:electron`으로 다시 확인한다.

### Unit 3.6.5 — 5. 알려진 제한사항

- 화면 맞춤은 현재 한 페이지 Canvas를 가용 화면 안에 넣는 표시 기능이다. 문제 단위 이동·문제별 PDF 주석·자동 문제 분리는 하지 않는다.
- 새 패키지 생성과 패키지 Electron 검증은 실행 중인 앱을 종료한 뒤 별도로 수행해야 한다.

### Unit 3.6.5 — 6. Git Commit Message

제안: `PDFolio repository at unit 3.6.5_reorganize viewer workspace and screen fit`
